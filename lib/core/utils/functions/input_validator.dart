...
extension InputValidator on Functions {
  String? nameValidator(String? value, BuildContext context) {
    if (value == null || value.isEmpty) {
      return JsonConsts.required.t(context);
    } else if (value.length < 2) {
      return JsonConsts.shortName.t(context);
    } else if (value.length > 20) {
      return JsonConsts.longName.t(context);
    } else {
      return null;
    }
  }

  String? multiLineValidator(String? value, BuildContext context) {
    if (value == null || value.isEmpty) {
      return JsonConsts.required.t(context);
